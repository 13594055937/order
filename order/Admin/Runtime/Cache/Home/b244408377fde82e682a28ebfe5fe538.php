            ...
            <div class="row">
                ...
                <div style="text-align:left" class="col-lg-12 m-b-xs">
                    <form class="form-inline" role="form" method="post">
                        <input type="hidden" id="out_g_order_action_url" value="<?php echo U('engineers/daochu');?>">
                        <input type="hidden" id="search_g_order_action_url" value="<?php echo U('engineers/index');?>">
                        <div class="form-inline" role="form">
                            <div>
                                <div class="form-group ">编号:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input value="<?php echo ($post["engineerCode"]); ?>" name="engineerCode" type="text" class="form-control" placeholder="单行输入"></div>
                                <div class="form-group ">状态:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    <select name="status" class="form-control">
                                        <option value="">全部</option>
                                        <option value="1" <?php if($post['status'] == 1): ?>selected<?php endif; ?> >可服务</option>
                                        <option value="0" <?php if($post['status'] != '' && $post['status'] == 0): ?>selected<?php endif; ?> >不可服务</option>
                                    </select>
                                </div>
                                <div class="form-group">&nbsp;&nbsp;&nbsp;姓名:&nbsp;&nbsp;&nbsp;&nbsp;<input value="<?php echo ($post["engineerName"]); ?>" name="engineerName" type="text" class="form-control" placeholder="单行输入" name="name"></div>
                            </div>
                            <div>
                                <div class="form-group">微信号:&nbsp;&nbsp;&nbsp;<input value="<?php echo ($post["weixin"]); ?>" type="text" name="weixin" class="form-control" placeholder="单行输入" name="name"></div>
                                <div class="form-group">服务区域:&nbsp;
            ...
            $.ajax({
                ...
                'success':function(msg){
                    $("#areainfo").html(msg);
                            ...
                            <div class="form-group">联系方式:<input type="text" name="mobile" value="<?php echo ($post["mobile"]); ?>" class="form-control" placeholder="单行输入" name="name"></div>
                            <div class="form-group">人员分类:
                                <select name="type" class="form-control">
                                    <option value="">全部</option>
                                    <?php if(is_array($engineertype_list)): foreach($engineertype_list as $key=>$engineer_type): ?><option value="<?php echo ($engineer_type["name"]); ?>" <?php if($post['type'] == $engineer_type['name']): ?>selected<?php endif; ?>><?php echo ($engineer_type["name"]); ?></option><?php endforeach; endif; ?>
                                </select>
                            </div>
                            <div class="form-group"><button class="btn btn-sm" id="add_g_order"><a href="<?php echo U('engineers/add');?>">新增</a></button></div>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        <?php if(is_array($list)): foreach($list as $k=>$data): ?><tr class="gradeA">
                                                <td><?php echo ($data["engineercode"]); ?></td>
                                                <td><?php echo ($data["engineername"]); ?></td>
                                                <td><?php echo ($data["mobile"]); ?></td>
                                                <td><?php echo ($data["openid"]); ?></td>
                                                <td><?php echo ($data["province"]); echo ($data["city"]); echo ($data["area"]); ?></td>
                                                <td><?php echo ($data["stardfee"]); ?></td>
                                                <td><?php if($data['status'] == 1): ?>可服务<?php else: ?>不可服务<?php endif; ?></td>
                                                <td><?php echo ($data["memo"]); ?></td>
                                                <td><a href="<?php echo U('engineers/add',array('id'=>$data['id'],'edit_info'=>'修改信息'));?>">修改</a></td>
