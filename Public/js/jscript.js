...
function SendSms(mobile, docuHid, docuBtn) {
    ...
    if ($.trim(mobile) == "") { alert("请先填写手机号码!");_IsDisabled = true;  return; }
    docuHid.attr("value", "");
    var numRandom = random(6);
    if (!validate_phone(mobile)) {
      alert("* 请填写正确的手机号码,例如:13415764179!");_IsDisabled = true; return false;
  }
  _IsDisabled == false;
    //docuBtn.attr("disabled", true);
    $.ajax({
        type: "Post",
        url: "/ajaxPage/ajaxApp.aspx/SendMessage",  //ajaxDataMethod是请求页面的方法名   
        //方法传参的写法一定要对，CityID为形参的名字      
...
var wait = 60;
function time(o, docuHid) {
    if (wait == 0) {//				o.removeAttribute("disabled");			
        //				o.value="免费获取验证码";
        _IsDisabled = true;
        o.html("获取短信验证码");
        //docuHid.attr("value","");
