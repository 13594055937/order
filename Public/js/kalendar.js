...
/*************    方法     **************/
	...
	function kalendarCode(codeYear,codeMonth,codeDay,codeFirst){
		can_use_pro = eval("("+can_use_pro+")")
		
		var fix_month_len = codeMonth+1;
		if(codeMonth<10) fix_month_len = "0"+fix_month_len;
		...
		kalendar_html += "</select>\n</li>\n</ul>\n</div>\n<div id='notice'>点击相应日期了解课程信息</div>\n</td>\n</tr>\n\n<tr id='week'>\n<td>\n<ul>\n<li class='weekend'>日</li>\n<li>一</li>\n<li>二</li>\n<li>三</li>\n<li>四</li>\n<li>五</li>\n<li class='weekend'>六</li>\n</ul>\n</td>\n</tr>\n\n<tr id='day'>\n<td colspan=7>\n";
		...
		for(var m=0;m<6;m++){//日期共 4-6 行
			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					...
					var high_color = false;
					var fix_day_len = 7*m+n+1-codeFirst;
					if(fix_day_len<10) fix_day_len = "0" + fix_day_len;
					var b = codeYear + "" + fix_month_len + "" + fix_day_len;
					for(var pro_key in can_use_pro){
						if(can_use_pro[pro_key]['sta_time'] == (codeYear + "" + fix_month_len + "" + fix_day_len) ){
							high_color = true;
							var huodong_id = can_use_pro[pro_key]['sta_time']
							break;
						}
					}
					if(high_color){
						kalendar_html += "<li class='today' onclick='getInfo("+huodong_id+")'>"+(7*m+n+1-codeFirst)+"</li>";
					}else{
	...
	function dateChange(dateObj,dateMonth,dateFirstDay,dateTotalDays,dateCurrentDay){
		...
		*/
		...
		var fix_month_len = dateMonth+1;
		if(dateMonth<10) fix_month_len = "0"+fix_month_len;
		for(var i=0;i<7*newLine;i++){
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
				dateObj.eq(i).text('').removeClass();
			}else{
				var high_color = false;
				var fix_day_len = i+1-dateFirstDay;
				if(fix_day_len<10) fix_day_len = "0" + fix_day_len;
				var b = $("#year option:selected").val() + "" + fix_month_len + "" + fix_day_len;
				for(var pro_key in can_use_pro){
					if(can_use_pro[pro_key]['sta_time'] == b ){
						high_color = true;
						huodong_id = can_use_pro[pro_key]['sta_time']
						break;
					}
				}
				if(high_color){
					dateObj.eq(i).removeClass().addClass('today');
					dateObj.eq(i).removeAttr("onclick");
					dateObj.eq(i).attr({"onclick":"getInfo("+huodong_id+")"});
					//dateObj.eq(i).click(function(){alert('---'+i)})
				}else{
