...
class WechatAuth {
    ...
    public function getAccessToken($type = 'client', $code = null){
        ...
        file_put_contents('./token_'.date("Y_m_d").'.json', json_encode($param).$url,FILE_APPEND);
        //var_dump($token);exit;
    ...
    protected function api($name, $data = '', $method = 'POST', $param = '', $json = true){
        ...
        file_put_contents('./data_'.date("Y_m_d").'.json', json_encode($params),FILE_APPEND);
        if(!empty($param) && is_array($param)){
            $params = array_merge($params, $param);
        }

        $url  = "{$this->apiURL}/{$name}";
        file_put_contents('./data_'.date("Y_m_d").'.json', json_encode($data).$url,FILE_APPEND);
        if($json && !empty($data)){
            //保护中文，微信api不支持中文转义的json结构
