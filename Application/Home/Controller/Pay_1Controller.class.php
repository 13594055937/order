...
class PayController extends Controller{
    ...
    public function _initialize() {
        ...
        import('Alipay.Corefunction');
        import('Alipay.Md5function');
        import('Alipay.Notify');
        import('Alipay.Submit');    
    }
    ...
    public function doalipay(){
       ...
       //这里我们通过TP的C函数把配置项参数读出，赋给$alipay_config；
        ...
        //建立请求
        $alipaySubmit = new \Org\Alipay\Submit($alipay_config);
    ...
    */
    public function returnurl(){
        ...
        $alipay_config=C('alipay_config');
        $alipayNotify = new \Org\Alipay\AlipayNotify($alipay_config);//计算得出通知验证结果
    ...
    public function orderhandle($parameter){
        $log_name = "notify_url2.log";//log文件路径
            $this->log_result($log_name, "【接收到的alipay notify通知】:\nccc\n");
	...
	$this->log_result($log_name, "【接收到的alipay notify通知】:\n".var_export($parameter,true));
        $data['status']     = 1;
        $orders = explode("_", $ordid);
        $order_id = $orders[1];
	$Ord = M('Data_trade');
	$this->log_result($log_name, "【接收到的alipay notify通知】:\n".$order_id);
        $Ord->where("order_id='".$order_id."'")->save(array(
                        'status'=> 1,
                        'pay_type'=>'支付宝',
